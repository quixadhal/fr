    ...
    int num_arg, instruction;
{
    ...
    *(arg--) = *(fp--); /* move source object to just below the pattern */
...
int num_arg, instruction;
{
   ...
   * allocate stack frame for rvalues and return value (number of matches);
   * perform some stack manipulation;
   * note: source and template strings are already on the stack by this time
   */
  fp = sp;
  sp += num_arg + 1;
  *sp = *(fp--);        /* move format description to top of stack */
  *(sp-1) = *(fp);      /* move source string just below the format desc. */
  fp->type = T_NUMBER;  /* this svalue isn't invalidated below, and if
			   we don't change it to something safe, it will
			   get freed twice if an error occurs */
  /*
   * prep area for rvalues
   */
  for (i = 1; i <= num_arg; i++)
