...
char *term;
{
    ...
    while (1) {
        ...
        if ((!strncmp(array_line[startchunk] + startpos, term, termlen)) &&
            ...
            /*
             * make sure there's room in the current chunk for terminator
             * (ie it's simpler if we don't have to deal with a terminator
             *  that spans across chunks)
             * fudge for "\",\"TERMINAL?\0", where '?' is unknown
             */
    ...
    char *str;
{
    ...
    if (simul && (simul->type & (TYPE_MOD_STATIC | TYPE_MOD_PRIVATE))) simul=0;
    ...
    if (strcmp(str,"efun")==0) return L_EFUN;
    if (strcmp(str,"asm")==0) return L_ASM;
...
static void add_instr_name(name, n, t)
    char *name;
    int n;
    short t;
{
...
void init_num_args()
{
    ...
    /* eoperators have a return type now.  T_* is used instead of
       TYPE_* since operators can return multiple types. */
