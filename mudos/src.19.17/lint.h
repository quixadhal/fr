...
void exit PROT((int));
	...
	!defined(linux) && !defined(sun) && !defined(sgi)
...
void free PROT((void *));
	...
	!defined(__bsdi__) && !defined(LATTICE) && !defined(ultrix)
...
int get_config_item PROT((struct svalue *res, struct svalue *arg));
...
int inter_sscanf PROT((struct svalue *, struct svalue *, struct svalue *, int num_arg));
...
void add_function PROT((struct function *, int *, int));
#endif
