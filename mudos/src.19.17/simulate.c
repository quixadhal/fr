 ...
 */
struct object *load_object(lname, flags)
     char *lname;
     int flags;
{
  FILE *f;
...
#else
    /* Beek - all the flags refer to this file and shouldn't be passed down */
    load_object(tmp, 0);		/* Remove this feature for now */
#endif
    FREE(tmp);
    
    /* Yes, the following is necessary.  It is possible that when we loaded
     * the inherited object, it loaded this object from it's create function.
     * Without this check, that would crash the driver.
     * -Beek
     */
     ...
     struct object *ob;
{
  ...
  if (ob->shadowed && !ob->shadowing) {
    ...
    for (ob2 = ob->shadowed; ob2->shadowed; ob2 = ob2->shadowed)
        ;
    /*
     * remove shadows top...down
     * being careful to unlink shadow being destructed from chain
     */
    for ( ; ob2; ) {
        svp.u.ob = ob2;
        ob2 = ob2->shadowing;
...
static void error_handler() { /* message is in emsg_buf */
