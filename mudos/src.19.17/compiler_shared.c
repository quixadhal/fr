...
char *get_two_types(type1, type2)
    int type1, type2;
{
 ...
 */
void add_arg_type(unsigned short type)
{
 ...
 */
int define_new_function(name, num_arg, num_local, offset, flags, type)
    char *name;
...
void define_variable(name, type, flags)
    char *name;
    int type;
    int flags;
{
...
int
validate_function_call(struct function *funp, int f, int num_arg) {
  if (funp->flags & NAME_UNDEFINED)
    find_inherited(funp);
  /*
