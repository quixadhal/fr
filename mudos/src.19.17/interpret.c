...
struct svalue catch_value = { T_NUMBER };
...
#else
  sprintf(buff, "AURORA MudOS %s%s-A", VERSION, PATCH_LEVEL);
#endif
}
     ...
     struct object *ob;
{
  ...
  /*
   * Want to iterate over the smaller function table, and use
   * binary search on the larger one for faster operation, ie
   *   O(m lg n), where m < n, versus O(m n)
   */
     ...
     struct svalue *v;
{
  ...
  v->type = T_FREED;
#endif
#if 0 /* Beek - this line alone increases CPU by a few % */
...
void bad_argument(struct svalue *val, int type, int arg, int instr) {
  char *buf;
  int flag = 0;
  ...
  while (1) { /* used to be the 'again' label */
   ...
   if (!is_efun)
    ...
    case I(F_LOCAL_NAME) :
      {
        ...
        /*
         * If variable points to a destructed object, replace it with 0,
         * otherwise, fetch value of variable.
         */
		...
		case T_STRING:
		  {
		    ...
		    (void)strcpy(res + r, sp->u.string);
		    /* we know they are strings so this saves time */
	...
	if (instruction == F_ADD_EQ) { /* not void add_eq */
	  ...
	  /* but if (void)add_eq then no need to produce an rvalue */
	  sp--;
	}
      }
      break;
    ...
    case I(F_VOID_ASSIGN) :
        if ((sp-1)->type == T_STRING) {
            if ((sp-1)->subtype == STRING_MALLOC ||
                ...
                /*
                 * avoid unnecessary (and costly) string_copy()...FREE()
                 *   or ref_string()...free_string()
                 */
...
struct vector *get_svalue_trace()
{
  ...
  for (p = &control_stack[0]; p < csp; p++) {
    ...
    if (p[0].funp) {
      ...
      for (i=0; i<n2; i++) {
	assign_svalue(&v2->item[i],&ptr[i+n]);
      }
  ...
  if (p[0].funp) {
    ...
    v2 = allocate_array(n2);
    for (i=0; i<n2; i++) {
      assign_svalue(&v2->item[i],&fp[i+n]);
    }
    add_mapping_array(m, "locals", v2);
    v2->ref--;
  }
...
int
inter_sscanf(arg, s0, s1, num_arg)
struct svalue *arg;
struct svalue *s0;
struct svalue *s1;
int num_arg;
{
  ...
  for (number_of_matches=0; num_arg >= 0; number_of_matches++) {
    ...
    if (fmt[0] == '%') {
      switch (fmt[1]) {
        case 's':
          error("Illegal to have 2 adjacent %%s's "
                "in format string in sscanf()\n");
        case 'd':
