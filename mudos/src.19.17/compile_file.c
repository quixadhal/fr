...
/* Needed by AIX to avoid name clash */
#undef F_EXEC
...
# endif
  ...
  if ((p = (char *)rindex(module_name, '/') + 1) == (char *) 1)
    p = module_name;
  ...
  (void) strcpy(symbol_name, "___FUNCS");
#ifndef HAS_NLIST
...
void
compile_file_error(int err, char *routine) {
  char *why;
  char buf[1024];
...
int generate_source(char *lname, char *out_fname) {
  FILE *crdir_fopen();
  FILE *f, *f_out;
