...
// Edited 1 JUN 95 by Manshoon to not take worn or wielded equipment.
...
int scaled_value(int n)
{
 ...
 int check_inv(object thing)
{
     int i;
   object *inv;
      inv = find_match(thing->query_short(),this_player());
...
int sell(string str) {
  ...
  if(!obs[i]->query_in_use())
    ...
    /* O.K. this SHOULD check to see if the item IS in the inventory before we move it */
         if(check_inv(obs[i])) {
