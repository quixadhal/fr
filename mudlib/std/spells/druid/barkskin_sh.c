...
int adjust_hp( int amount, object hitter)
{
  ...
  if ( hitter )
    tell_object(hitter, "Your attack glances off of "+
      my_player->query_cap_name()+ "'s tough, bark-like skin.\n");
  ...
  if ( check_duration() )
    call_out("destruct_barkskin_shadow",0,0);
...
int set_hp(int amount, object hitter)
{
  ...
  if ( hitter )
    tell_object(hitter, "Your attack glances off of "+
      my_player->query_cap_name()+"'s bark-like skin.\n");
  tell_object(my_player, "The attack glances off your Barkskin.\n");
  tell_room(environment(my_player), "The attack "
