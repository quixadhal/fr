...
void do_spell(object ob)
{
   ...
   if( find_living((string)ob->query_name()) )
   {
	  ...
	  if(random((int)ob->query_dex()) < 8 )
	  {
	    ...
	    else ob->adjust_hp(-THORN_DAMAGE,this_object());
	     }
	 else 
	 {
	   if(!interactive(ob)) ob->adjust_hp(-THORN_DAMAGE,this_object());
	    else if(ob->query_hp() > THORN_DAMAGE) ob->adjust_hp(-THORN_DAMAGE,this_object());
	 }
	  }
