
/* Poison object, actually a "dead" invisible NPC 
 * Effects based on con, once cast this isn't a spell
 * but a festering wound
   * This is a VERY mild poison, a poisoned scratch.
 */ 
#include "tweaks.h"
inherit "/obj/monster";

object strength;
int hb;

int dispell_me()
{
   this_object()->dest_me();
   return 1;
}
void init()
{
   ::init();
   set_heart_beat(1);
}
void set_spell(object ob,int damage)
{
    strength = damage;
}

void setup()
{
   set_name("poisoned bite");
   set_short("poisoned bite");
   set_race("monster");
   add_alias("wound");
   set_long("\n A disgusting looking wound that is weeping greennish puss and making you feel quite ill"
   "\n");
   set_get();
   reset_drop();
   set_gender(0);
   strength = random(3)+1;
}
void heart_beat()
{
     object ob;
     int i;
     ::heart_beat();
      hb++;
     
     ob = environment(this_object());
     if(!ob || ob->query_property("dead") || !ob->query_timed_property("poisoned") )
     {
	  dispell_me();
     } 
     if(hb > ob->query_con() )
     {
	hb = 0;
	ob->adjust_hp(-random(strength),this_object());
   tell_object(ob,"The poisoned bite burns, and you feel weaker.\n");
   tell_room(environment(ob),(string)ob->query_cap_name()+" suddenly looks uncomfortable.\n",({ ob }) );
     }
   set_heart_beat(1);
}     
