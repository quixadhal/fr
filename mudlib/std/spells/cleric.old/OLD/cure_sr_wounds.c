...
mixed spell(string str, int skill, int cast)
{
  ...
  if (sizeof(ob))
    ...
    ob = 0;
  this_player()->add_spell_effect(1, "curative", "Cure Serious Wounds",
...
int hb_spell(object attacker, mixed *params)
{
  ...
  if(previous_object() == params[1])
  {
    say((string)params[1]->query_cap_name()+" cures some of "+
     (string)params[1]->query_possessive()+" serious wounds.\n", params[1]);
    tell_object(params[1], "You cure some of your serious wounds.\n");
  }
  else
  {
