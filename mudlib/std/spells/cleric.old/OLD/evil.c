...
int cast_spell(string str)
{
 ...
 if (skill < MIN_LVL)
 {
  notify_fail("You are not skilled enough to cast Detect Evil.\n");
  return 0;
 }
 ...
 write("You start casting Detect Evil.\n");
 return 1;
}
...
mixed spell(string str, int skill, int cast)
{
 ...
 this_player()->add_spell_effect(1, "misc", "Detect Evil", this_object(),
...
int hb_spell(object caster, mixed *params)
{
 ...
 if(!params[1])
 {
  tell_object(previous_object(), "Your Detect Evil fizzles since you "
 ...
 {
  tell_object(previous_object(),"Don't you even know your own alignment ?\n");
 ...
 else
 {
  if((int)params[1]->query_al() < 0)
   tell_object(previous_object(),"You cast Detect Evil on "+
