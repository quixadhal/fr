...
mixed spell(string str, int skill, int cast)
{
  ...
  if (sizeof(ob))
    ...
    ob = 0;
  this_player()->add_spell_effect(1, "curative", "Remove Curse",
...
int hb_spell(object attacker, mixed *params)
{
  ...
  {
    tell_object(previous_object(),"You lift your curses.\n");
    say((string)previous_object()->query_cap_name() +
      " mumbles something, and immediatly looks happier.\n", params[1]);
  }
  else
  {
    tell_object(previous_object(), "You cast remove curse on " +
     (string)params[1]->query_cap_name() +
     ", who immediately looks happier.\n");
    say((string)previous_object()->query_cap_name() +
