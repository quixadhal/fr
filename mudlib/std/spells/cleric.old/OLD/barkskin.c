...
int cast_spell(string str)
{
  ...
  if (str==(string)this_player()->query_name())
   skill=(int)this_player()->query_skill_bonus("faith.spells.defensive.self");
  else
  ...
  write("You start to cast Barkskin.\n");
...
mixed spell(string str, int skill, int cast)
{
  ...
  this_player()->add_spell_effect(1, "defensive", "Barkskin",
...
int hb_spell(object attacker, mixed *params)
{
  ...
  say((string)previous_object()->query_cap_name() +
    ...
    ", whose skin starts to look barklike.\n", params[1]);
  if(params[1] == previous_object())
    tell_object(params[1], "You cast Barkskin on yourself.\n");
  else
  {
   tell_object(params[1], (string)previous_object()->query_cap_name() +
    " casts a spell on you and your skin takes on a barklike quality.\n");
  
   tell_object(previous_object(), "You cast Barkskin on " +
...
void wear_off(object target)
{
  if(target)
  {
   ...
   target->set_ac(ac-10);
   tell_object(target, "Your skin returns to normal.\n");
   tell_room(environment(target),(string)target->query_cap_name()+
   "'s skin return to normal.\n", ({target}));
  }
}
