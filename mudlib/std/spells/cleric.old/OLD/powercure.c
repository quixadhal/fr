...
mixed spell(string str, int skill, int cast)
{
  ...
  if (sizeof(ob))
    ...
    ob = 0;
  this_player()->add_spell_effect(1, "curative", "Powercure",
...
int hb_spell(object attacker, mixed *params)
{
  ...
  say((string)previous_object()->query_cap_name() +
    " Powercures " + (string)params[1]->query_cap_name() +
    "'s light wounds.\n", params[1]);
  tell_object(params[1], (string)previous_object()->query_cap_name() +
    " Powercures you.\n");
  tell_object(previous_object(), "You Powercure " +
