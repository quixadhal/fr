...
string powerp(int level,object ob,string str,string show)
{
  string temp;
  temp ="";
   if(!intp(ob->query_property(str))) return "";
...
string powerp1(int level,object ob,string str,string show)
{
  string temp;
  temp ="";
  if(!(int)ob->query_property(str)) return ("");
  switch((int)ob->query_property(str))
  {
    case -399..-1:
      temp = "A black aura of "+show+" ";
    break;
    case 1:
      temp = "A faint numbus of "+show+" ";
    break;
    case 2:
      temp = "A definate aura of "+show+" ";
    break;
    case 3:
      temp = "A steady glowing aura of "+show+" ";
    break;
    case 4:
      temp = "A solid aura of "+show+" ";
    break;
    case 5..10:
      temp = "A blinding aura of "+show+" ";
    break;
  }
  if(random(level) > 20) switch((int)ob->query_time_remaining(str))
  {
    case 0:
        temp += "and it's permanent.";
    break;
  ...
  if(living(target))
  {
    tell_object(caster,"You look closely and"
      ...
      "%^CYAN%^"+powerp(level,target,"air","lavendar")+"%^RESET%^"+
		"%^BOLD%^%^BLACK%^"+powerp(level,target,"stone","grey")+"%^RESET%^"+
		powerp(level,target,"psionic","mental strength")+
		powerp1(level,target,"str","orc")+
		powerp1(level,target,"con","troll")+
		powerp1(level,target,"dex","halfling")+
		powerp1(level,target,"int","gnome")+
		powerp1(level,target,"wis","dwarf")+
		powerp1(level,target,"cha","human")+
