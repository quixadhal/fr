/*** Major Magical Protection Spell ***/
/*** Created by Eerevann Tokani ***/


#define SP_NAME "Major Magical Resistance"
#define GP_COST 12
#define TYPE "magical"
...
string help() {
       ...
       "by 50% plus +1% per level of the caster.\n"+
       "    The duration is based upon the casters level.\n"+
     "NOTE: This spell only affects damage from "+
    "magical attacks,  so a fireball for example would "+
    "not be affected, but magic missile damage would.  Also "+
    "some miscellaneous spells duration is affected by "+
    "this spell.\n\n";
...
int hb_spell(object caster, mixed *params)
{
  ...
  say((string)previous_object()->query_cap_name()+
   " chants, 'marj maycal rissas'.\n", ({params[1],previous_object()}));
    
  if(params[1] == previous_object())
   { 
    tell_object(params[1], "You chant, 'marj maycal rissas'.\n");
  ...
  {
   tell_object(previous_object(), "You chant, 'marj maycal rissas'.\n");
   
   tell_object(params[1], (string)previous_object()->query_cap_name() +
    " chants, 'marj maycal rissas'.\n");
   
   tell_object(params[1], "You feel resistant to Magical attacks.\n");
  ...
  resist_value = level + 50;

   params[1]->add_timed_property(TYPE,resist_value,50+(level*3));
  return GP_COST;
}
