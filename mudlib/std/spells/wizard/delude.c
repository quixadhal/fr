 ...
 *
 * Changed to use an object rather than npc, wonderflug 95.
 */ 
#include "tweaks.h"
...
int dispell_me()
{
  tell_room(ETO,"The cloud dissapates leaving you feeling strange.\n",({ }));
  this_object()->dest_me();
  return 1;
}
...
void attack_by(object ob)
{
  ...
  tell_room(ETO,(string)ob->query_cap_name()+
    " bravely, but futilely, attacks a narcotic cloud, "
...
void do_spell(object ob)
{
  ...
  if(find_living((string)ob->query_name()) )
  {
    ...
    {
      case 0:   
        tell_object(ob,"A wave of euphoria rolls over you.\n");
        tell_room(ETO,(string)ob->query_cap_name()+" looks euphoric.\n",({ob}));
        ob->set_wimpy(100);
      ...
      case 9: /* will only get players L9 or greater */
      case 10:
        tell_object(ob,"A wave of confusion rolls over you.\n");
        tell_room(ETO,(string)ob->query_cap_name()+" looks confused.\n",({ob}));
        force_drop_one(ob);
...
void setup()
{
  set_name("cloud of delusion");
  set_short("cloud of delusion");
  add_alias("cloud");
