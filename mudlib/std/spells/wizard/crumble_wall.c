...
void hb_spell(object ob, mixed * params, int time);
...
int hb_spell(object caster, mixed *params, int time)
{
 ...
 int i;
  ...
  if ( time == 3 )
  {
    tell_room(environment(caster), "The walls begin to rumble and shake.\n");
    return 0;
  }

  if ( time == 2 )
  {
    tell_room(environment(caster), "The shaking suddenly stops, "+
      "and a dreadful silence cloaks the area.\n", ({ }) );
    return 0;
  }
 ...
 if (params[0] && (int)caster->adjust_gp(-GP_COST)<0)
 {
  ...
  if ( params[0] )
  {
    tell_room(environment(caster),str + " chants, 'retro lithos' and a wall explodes!\n",
        caster);
