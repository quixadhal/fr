...
mixed spell(string str, int skill, int cast)
{
  ...
  this_player()->add_spell_effect(1, "attack", "Burning Hands", this_object(),
 ...
 if (params[2] && (int)previous_object()->adjust_gp(-GP_COST)<0)
 {
  ...
  while(i<sizeof(params[1])) {
       ...
       environment(params[1][i]) == environment(previous_object()))
    {
      ...
      tell_object(params[1][i], str + " fan you with a burning arch!\n");
  ...
  say(str + " fans "+(str = query_multiple_short(params[1]))+
   " with a burning arch.\n", params[1]);
 tell_object(previous_object(), "You cast Burning Hands at " + str + ".\n");
