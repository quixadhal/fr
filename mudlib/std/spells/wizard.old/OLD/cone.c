...
int cast_spell(string str)
{
  ...
  write("You cast your Cone of Cold.\n");
  return 1;
}
...
mixed spell(string str, int skill, int cast)
{
  ...
  if (sizeof(ob))
    ...
    ob = 0;
  this_player()->add_spell_effect(1, "attack", "cone of cold", this_object(),
    "hb_spell", ({ skill*3/2,ob,cast }));
  return 1;
}
...
int hb_spell(object attacker, mixed *params)
{
 ...
 say((string)previous_object()->query_cap_name()+" casts Cone of Cold at "+
  (string)params[1]->query_cap_name()+".\n", params[1]);
 tell_object(params[1], (string)previous_object()->query_cap_name()+
  " casts Cone of Cold at you.\n");
 tell_object(previous_object(), "You cast Cone of Cold at "+
  (string)params[1]->query_cap_name()+".\n");
 params[1]->attack_by(previous_object());
/* exp... */
 hp = (int)params[1]->query_hp();

 return (hp-(int)params[1]->adjust_hp(-params[0]))+50;
}
