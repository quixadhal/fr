...
int round1(object caster, mixed target, mixed out_range, int time, int quiet)
{
  ...
  if ( !quiet )
  {
    ...
    tell_object(caster, "You chant, 'magius criusis harmei'.\n");
  }
  ...
  if((random((int)caster->query_thac0()) + target->query_total_ac()) > 0)
  {
    tell_object(target, (string)caster->query_cap_name()+
      " reaches out and touches you as some gaping, bleeding wounds "
      "appear on you.\n");
    tell_object(caster, "You cast Cause Critical Wounds and touch "+
      (string)target->query_cap_name() + " causing gaping, bleeding "
      "wounds to appear.\n");
    tell_room(environment(caster), (string)caster->query_cap_name()+
      " casts a spell and touches "+target->query_cap_name()+" causing "
      "gaping, bleeding wounds to appear.\n",
      ({target,caster}) );
  } 
  else 
  {
    ...
    tell_object(caster, "You cast Cause Critical Wounds and try to touch "+
