...
void setup()
{
  ...
  set_help_desc("This spell reduces damage done by Cold based attacks "
...
int round1(object caster, mixed target, mixed out_range, int time, int quiet)
{
  ...
  if(target == caster)
  { 
    tell_object(target, "You feel resistant to cold attacks.\n");
    tell_room(environment(caster), (string)caster->query_cap_name()+
      " seems to radiate an inner warmth.\n",target);
  }
  else
  {
    tell_object(target, "You feel resistant to Cold attacks.\n");
    tell_object(caster, "You cast Resist Cold on "+
