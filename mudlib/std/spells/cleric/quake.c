/* Quake */
/* Set max damage at 30th.  After that level, damage increases
	only marginally, (dam + level/2).  Upped base damage b/c of
	this, as I no longer have to code worrying about Deception's
	possible damage, (as it is that of a 30th lvl cyrcia now) */
...
int round3(object caster, mixed target, mixed out_range, int time, int quiet)
{
  ...
  if(here==0)
	{
	tell_object(caster, "Alas, you are too far from Cyrcia's"
		" domain, and her power fails you.\n");
	return -1;
}
...
num_dice = ((random(3) * level) + (caster->query_level()/2));
