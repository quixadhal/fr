...
void setup()
{
  ...
  set_help_desc("This spell reduces damage done by good based attacks "
...
int round1(object caster, mixed target, mixed out_range, int time, int quiet)
{
  ...
  if ( !quiet )
  {
    tell_room(environment(caster), (string)caster->query_cap_name()+
      " chants, 'Dexter rissas cleios'.\n", ({target,caster}));
    tell_object(caster, "You chant, 'Dexter rissas cleios'.\n");
  }
    
  if(target == caster)
  { 
    tell_object(target, "You feel resistant to good attacks.\n");
    tell_room(environment(caster), (string)caster->query_cap_name()+
      " seems to radiate an inner aura of evil.\n",target);
  }
  else
  {
    tell_object(target, "You feel resistant to good attacks.\n");
    tell_object(caster, "You cast Resist good on "+
