...
int meditate(string str)
{
  ...
  object env=environment(disc);
   ...
   {
      write("For some reason you feel so safe here that you can't possibly meditate.\n");
   return 1;
   }
  ...
  if ( disc->query_trance() )
  {
    ...
    tell_room(env, disc->query_cap_name()+" comes out of a meditative "
      "trance and stands up.\n", disc );
    disc->dest_trance();
    return 1;
  }
