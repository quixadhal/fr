...
int palm(string str) {
  ...
  another_meaningless_name = t_p = fail = g_f = ({ });
  ...
  for (i=0;i<sizeof(obs);i++)
    ...
    if ( this_player()->query_level() > obs[i]->query_weight() / (random(20)+5) )
        t_p += ({ obs[i] });
      else another_meaningless_name += ({ obs[i] });
  ...
  if (sizeof(another_meaningless_name)) {
