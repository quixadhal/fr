...
string help() 
{
  return
    ...
    "of Death, Grimbrand himself, demands half in payment.  This ability will "
    "heal a creature of undeath, but beware, for then the Knight himself "
    "shall have to make Grimbrand's dread payment.\n\n";  
...
mixed spell(string str, object caster)
{
  ...
  if (!sizeof(ob) || !living(ob=ob[0]) || ob->query_dead() )
