...
int peek(string str)
{
  ...
  if (!(target->query_hb_counter()))
  {
	write("You stare intently at an inanimate object.  Fascinating.\n");
	return 0;
  }
  if (environment(target) != environment(peeker))
  {
	notify_fail("Your target is not here.\n");
	return 0;
  }
  ...
  {
	case 1..1000:
	  write("You see the following items as you peek at "+
	    target->query_cap_name()+": \n"+
	    target->query_living_contents(0));
	  break;
	case -10..0:
	  write("You think "+target->query_cap_name()+" noticed you looking, "+
	    "but you saw the following items anyway: \n"+
	    target->query_living_contents(0));
	  ...
	  break;
	default:
	  write("It seems "+target->query_cap_name()+" noticed you staring at "+
	    target->query_objective()+", and you didn't get a good look at "+
	    "anything.\n");
	  tell_object(target,"You notice "+peeker->query_cap_name()+
	    " looking intently at your belongings.\n");
	  tell_room(environment(peeker), peeker->query_cap_name()+" gets "+
