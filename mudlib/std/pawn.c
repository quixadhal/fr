...
int leave_item(string str) {
  ...
  if(!((int)(ob[loop]->move(environment(ob[loop])))==3))
 {
    ...
    {
     write("You cannot sell "+ob[loop]->query_short()+".\n");
    }
 }
 if(money_to_give)
 {
  ...
  {
   write("The pawnbroker screams: Get out of my shop you DEADBEAT!\n");
   log_file("pawn_cheats",this_player()->query_name()+" tried to sell a reset_drop() item.\n");
  }
  return 1;
}
