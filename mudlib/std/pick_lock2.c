...
string alt_lock_name(object locd_ob);
 ...
 int toolbonus = this_object()->query_property("skill enhance");
   ...
   switch(g_name){
      ...
      case "thief":
         ...
         break;
      case "hokemj":
         pick_skill=10+random(20)+((level*3)/2)+((stat_int+stat_dex)/2);
         break;
      default:
...
string alt_lock_name(object locd_ob)
{
  int i;
  string lock_name;
  int lnum = locd_ob->query_num_locks();
  mixed *lock_list = locd_ob->query_lock_list();

  printf("\n%s%d%s","  It seems that there are ",lnum," locks.");
  printf("\n%s","  Which lock do you want to pick:");
  for(i=0; i<sizeof(lock_list); i++)
  {
    printf("\n%s%s","       ",(string)lock_list[i]);
  }
  input_to("alt_lock_name_too");
  return lock_name;
}
