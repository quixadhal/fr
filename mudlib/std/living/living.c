...
int do_hold(string woo)
  ...
  // boo = find_match(woo, this_player());
  boo = all_inventory(this_player());
...
int do_unhold(string woo)
  ...
  // -Aragorn
  boo -= (all_inventory(this_player())-(mixed*)this_object()->query_held_ob());
...
int do_unwear(string woo)
  ...
  // right now. Speedups later.
  // -Aragorn
  boo -= (all_inventory(this_player())-(mixed*)this_object()->query_worn_ob());
...
int query_total_ac()
  {
  /* AC 10 is the base, all ac is subrtacted from this to getthe AD&D ac */
  return ( 10 - (query_equip_ac() + query_body_ac()) );
  }
