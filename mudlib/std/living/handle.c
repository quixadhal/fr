...
#define TIME_DIV 5
#define HOLD_CODE "/std/living/hold.c"
#define WEAR_CODE "/std/living/hold.c"
...
/* should check to see that we can give things to this person/thing... */
  ...
  for (;j<sizeof(per);j++) {
    ...
    for (i=0;i<sizeof(obs);i++)
      ...
      /* Add by Baldrick, so the players willl unhold the object they give..
       ...
       */
        if (obs[i]->query_in_use() && obs[i]->query_holdable() )
          HOLD_CODE->unhold_ob(obs[i]);

        if ( obs[i]->query_in_use() && obs[i]->query_wearable() )
          WEAR_CODE->unwear_ob(obs[i]);
...
int drop_ob(string str) 
  ...
  for (i=0;i<sizeof(ob);i++) 
    ...
    if (ob[i]->query_in_use() && ob[i]->query_holdable() )
      HOLD_CODE->unhold_ob(ob[i]);

    if ( ob[i]->query_in_use() && ob[i]->query_wearable() )
      WEAR_CODE->unwear_ob(ob[i]);
