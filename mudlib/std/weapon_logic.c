/*
   iew weapon Logic by Sojan.  Designed to implement weapons in a such a way
...
void calc_damage(int dam_bon)
{
 int damage;
 
 damage=this_object()->query_damage_roll()+dam_bon;
 return damage;
}
...
//DEBUG LINE FOLLOWS   
 //   tell_object(attacker, sprintf("DEBUG: Roll is %d.\n", tmproll));
   ...
   /* Adding the 100% hit on a roll of 20.
    * Baldrick, oct '94.
    */
   if (tmproll==20)
     {
   ...
   //achit = ((THAC0 - bon - (int)attacker->extra_bonus("attack")) - tmproll);
...
//DEBUG LINE FOLLOWS
   ...
   if (result < 1) 
     {
     happen = "hit";
      damage_done = calc_damage((int)attacker->query_damage_bonus());
     }
   else 
     {
     happen = "miss";
