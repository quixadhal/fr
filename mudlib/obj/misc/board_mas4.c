// Added selective 'new' list commands
//    addnew <boardname> 
//    removenew <boardname>
...
void init() {
  ...
  add_action("newnew", "new");
  add_action("add_selective","addnew");
  add_action("remove_selective","removenew");
...
int newnew() {
  ...
  boards = (string *)this_player()->query_property("selective_rc");
  if(!sizeof(boards))
     boards = (string *)BOARD_HAND->list_of_boards();
...
int add_selective(string str)
{
   int i;
   string *boards;
   if(!str)
   {
      notify_fail("Syntax: addnew <board name>\n");
      return(0);
   }
   boards = (string *)BOARD_HAND->list_of_boards();
   if((i=member_array(str, boards)) == -1)
   ...
   boards =  (string *)this_player()->query_property("selective_rc");
   if(!sizeof(boards)) boards = ({ });
   if((i=member_array(str,boards)) != -1)
   {
      notify_fail("The board "+str+" was already in your 'new' list.\n");
      return(0);
   }
   write("Added board "+str+" to your 'new' list.\n");
   this_player()->remove_property("selective_rc");
   boards += ({ str });
   this_player()->add_property("selective_rc",boards);
   return(1);
}
int remove_selective(string str)
{
   int i;
   string *boards;
   if(!str)
   {
      notify_fail("Syntax: removenew <board name>\n");
      return(0);
   }
   boards =  (string *)this_player()->query_property("selective_rc");
   if((i=member_array(str,boards)) == -1)
   {
      notify_fail("The board "+str+" was not in your 'new' list.\n");
      return(0);
   }
   write("Removed board "+str+" from your 'new' list.\n");
   this_player()->remove_property("selective_rc");
   boards -= ({ str });
   this_player()->add_property("selective_rc",boards);
   return(1);
