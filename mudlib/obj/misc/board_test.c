...
/* Wonderflug 1995, you guessed it, extended it to full read/write */
static string *readonly = ({"playerinfo","quests","announcements"});
static string* writeonly = ({ "Assassin_contact" });
...
int query_read_permission(object me)
{
  if( (member_array(board_name,writeonly) != -1) && 
      !me->query_creator())
    return 0;
  return 1;
}

int query_write_permission(object me)
{
  if ( (member_array(board_name,readonly) != -1) &&
       !me->query_creator())
    return 0;
  return 1;
}
...
string query_plural() 
{
  ...
  if ( !this_object()->query_read_permission(this_player()) )
...
int subjects(string str, int dark) 
{
  ...
  if ( !this_object()->query_read_permission(this_player()) )
  {
    notify_fail("You cannot read this board.\n");
    return 0;
  }
  stuff = (mixed *)BOARD_HAND->get_subjects(board_name);
  if (!sizeof(stuff)) 
...
string long(string str, int dark) 
{
  ...
  if (!sizeof(stuff) || !this_object()->query_read_permission(this_player()) )
...
int post(string str) 
{
  ...
  if( !this_object()->query_write_permission(this_player()) )
  {
    notify_fail("You cannot write on this board.\n");
    return 0;
  }
