...
({
  ...
  "kiss" : ({ ({ 0, "%s", 1, "%s %s", 2, "%s %s" }), ({
      ...
      "lovingly,lengthily,on the nose,fishily,carefully,demandingly,sensuously$~$",
...
find_all_liv(str) {
  ...
  /* This time it's out for good ! dunno why I don't delete the whole darn 
     thingie... Baldrick */
  /*
  if (str == "everyone") {
    ob = users();
  ...
  if ((ob = find_living(str)))
    if (member_array(ob, ret) == -1 && !ob->query_invis())
