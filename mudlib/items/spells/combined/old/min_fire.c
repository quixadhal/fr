/*** Minor Fire Protection Spell ***/
/*** Created by Eerevann Tokani ***/


#define SP_NAME "Minor Fire Resistance"
#define GP_COST 6
#define TYPE "fire"
...
int hb_spell(object attacker, mixed *params)
{
  ...
  say((string)previous_object()->query_cap_name()+
   " chants, 'mior pyros rissas'.\n", ({params[1],previous_object()}));
    
  if(params[1] == previous_object())
   { 
    tell_object(params[1], "You chant, 'mior pyros rissas'.\n");
  ...
  {
   tell_object(previous_object(), "You chant, 'mior pyros rissas'.\n");
   
   tell_object(params[1], (string)previous_object()->query_cap_name() +
    " chants, 'mior pyros rissas'.\n");
   
   tell_object(params[1], "You feel slightly resistant to Fire attacks.\n");
