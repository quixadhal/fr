/*** Minor Magical Protection Spell ***/
...
int hb_spell(object attacker, mixed *params)
{
  ...
  say((string)previous_object()->query_cap_name()+
   " chants, 'mior maycal rissas'.\n", ({params[1],previous_object()}));
    
  if(params[1] == previous_object())
   { 
    tell_object(params[1], "You chant, 'mior maycal rissas'.\n");
  ...
  {
   tell_object(previous_object(), "You chant, 'mior maycal rissas'.\n");
   
   tell_object(params[1], (string)previous_object()->query_cap_name() +
    " chants, 'mior maycal rissas'.\n");
   
   tell_object(params[1], "You feel slighly resistant to Magical attacks.\n");
