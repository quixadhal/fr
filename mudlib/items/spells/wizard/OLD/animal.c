...
int cast_spell(string str)
{
 ...
 if (skill < MIN_LVL)
 {
  notify_fail("You are not skilled enough to cast Summon Animal.\n");
  return 0;
 }
...
mixed spell(string str, int skill, int cast)
{
 ...
 this_player()->add_spell_effect(1, "offensive", "Summon Animal", this_object(),
...
/* devil = clone_object("i");*/
  tell_object(previous_object(), "Your animal wanders off into the forest, "+
   "being unable to find its intended target.\n");
  return 0;
 }
 ...
 {
  tell_object(previous_object(),"Even though the animal feels it is "+
   "stupid to attack its own master it does your bidding, attacking you.\n");
  say((string)previous_object()->query_cap_name() +
   " mumbles some strange words, and is attacked by an animal.\n", params[1]);
 }

 else
 {
  tell_object(previous_object(),"You cast Summon Animal on "+
   (string)params[1]->query_cap_name()+".\n");
  say((string)previous_object()->query_cap_name()+ " mumbles something and "+
   (string)params[1]->query_cap_name()+" is attacked by an animal.\n",
    params[1]);
  tell_object(params[1], (string)previous_object()->query_cap_name()+
   " mumles something and you are attacked by an animal.\n");
 }

 devil = clone_object("/std/spells/wizard/animal_obj");
 devil->move(environment(params[1]));
 devil->do_command("growl "+(string)params[1]->query_name());
