...
int cast_spell(string str)
{
 ...
 level = (int)this_player()->query_skill("magic.spells.offensive.target");
  ...
  write("You cast your Meteor Swarm.\n");
  return 1;
}
...
mixed spell(string str, int skill, int cast)
{
  ...
  times = (int)this_player()->query_skill_level("magic.spells.offensive.target")/50;
  if (times < 2) times = 2;
  if ((int)this_player()->query_spell_effect("attack"))
    return "You are already casting an offensive spell.\n";
  ob = find_match(str, environment(this_player()));

 this_player()->add_spell_effect(times, "attack", "Meteor Swarm", this_object(),
 ...
 if (params[2] && (int)previous_object()->adjust_gp(-GP_COST)<0)
 {
  ...
  while(i<sizeof(params[1])) {
       ...
       environment(params[1][i]) == environment(previous_object()))
    {
      ...
      tell_object(params[1][i], str + " showers you with meteors!\n");
  ...
  if(!sizeof(params[1])) {
    notify_fail("Noone to swarm with meteors here.\n");
    return 0;
  }
  say(str + " casts a Meteor Swarm at " +
    (str = query_multiple_short(params[1])) +".\n", params[1]);
 tell_object(previous_object(), "You cast Meteor Swarm at " + str + ".\n");
