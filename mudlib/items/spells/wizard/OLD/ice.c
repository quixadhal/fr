...
int cast_spell(string str)
{
  ...
  write("You prepare for casting an Ice Storm.\n");
  return 1;
}
...
mixed spell(string str, int skill, int cast)
{
  ...
  this_player()->add_spell_effect(1, "attack", "Ice Storm", this_object(),
    "hb_spell", ({ skill*4/3,ob,cast }));
  return 1;
}
 ...
 if (params[2] && (int)previous_object()->adjust_gp(-GP_COST)<0)
 {
  ...
  while(i<sizeof(params[1])) {
       ...
       environment(params[1][i]) == environment(previous_object()))
    {
      ...
      tell_object(params[1][i], str + "  throws an Ice Storm at you!\n");
  ...
  if(!sizeof(params[1])) {
    notify_fail("Noone to cast Ice Storm on here.\n");
    return 0;
  }
  say(str + " casts an Ice Storm at " +
    (str = query_multiple_short(params[1])) +".\n", params[1]);
 tell_object(previous_object(), "You cast Ice Storm at " + str + ".\n");
