...
int cast_spell(string str)
{
  ...
  write("You start preparing a Lightning Bolt.\n");
  return 1;
}
...
mixed spell(string str, int skill, int cast)
{
  ...
  if (sizeof(ob))
    ...
    ob = 0;
  this_player()->add_spell_effect(1, "attack", "Lightning Bolt", this_object(),
...
int hb_spell(object attacker, mixed *params)
{
 ...
 say((string)previous_object()->query_cap_name()+" casts a Lightning Bolt at "+
  (string)params[1]->query_cap_name()+".\n", params[1]);
 tell_object(params[1], (string)previous_object()->query_cap_name()+
  " casts a Lightning Bolt at you.\n");
 tell_object(previous_object(), "You cast Lightning Bolt at "+
  (string)params[1]->query_cap_name()+".\n");
