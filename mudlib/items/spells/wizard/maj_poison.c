/*** Major Poison Protection Spell ***/
/*** Created by Eerevann Tokani ***/


#define SP_NAME "Major Poison Resistance"
#define GP_COST 14
#define TYPE "poison"
...
int hb_spell(object attacker, mixed *params)
{
  ...
  say((string)previous_object()->query_cap_name()+
   " chants, 'marj poyze rissas'.\n", ({params[1],previous_object()}));
    
  if(params[1] == previous_object())
   { 
    tell_object(params[1], "You chant, 'marj poyze rissas'.\n");
  ...
  {
   tell_object(previous_object(), "You chant, 'marj poyze rissas'.\n");
   
   tell_object(params[1], (string)previous_object()->query_cap_name() +
    " chants, 'marj poyze rissas'.\n");
   
   tell_object(params[1], "You feel resistant to Poison attacks.\n");
