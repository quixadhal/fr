/*** Minor Electrical Protection Spell ***/
/*** Created by Eerevann Tokani ***/


#define SP_NAME "Minor Electrical Resistance"
#define GP_COST 6
...
int hb_spell(object attacker, mixed *params)
{
  ...
  if(params[1] == previous_object())
   { 
    tell_object(params[1], "You chant, 'mior xlxyx rissas'.\n");
  ...
  {
   tell_object(previous_object(), "You chant, 'mior xlxyx rissas'.\n");
   
   tell_object(params[1], (string)previous_object()->query_cap_name() +
    " chants, 'mior xlxyx rissas'.\n");
   
   tell_object(params[1], "You feel slighly resistant to Electrical attacks.\n");
