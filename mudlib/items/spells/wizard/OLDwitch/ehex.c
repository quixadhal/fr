...
say((string)previous_object()->query_cap_name()+" points at "+
  ...
  (string)params[1]->query_cap_name()+" is bombarded with pure elemental forces.\n", params[1]);
 tell_object(params[1], (string)previous_object()->query_cap_name()+
 " points at you.\n"+"You are bombarded with pure elemental force.\n");
 tell_object(previous_object(), "You point at "+
  (string)params[1]->query_cap_name()+".\n"+"You call upon the powers"+
 " of all of the elements to hex "+(string)params[1]->query_cap_name()+".\n"+
 (string)params[1]->query_cap_name()+" is bombarded with pure elemental "+
 "force.\n");
params[1]->attack_by(previous_object());
/* exp... */
 hp = (int)params[1]->query_hp();
return (hp-(int)params[1]->adjust_hp(-params[0]));
