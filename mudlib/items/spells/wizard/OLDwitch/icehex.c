...
int cast_spell(string str)
{
  ...
  write("You start the gestures necessary for a complex hex.\n");
  return 1;
}
...
if (params[2] && (int)previous_object()->adjust_gp(-GP_COST)<0)
 ...
 say((string)previous_object()->query_cap_name()+" points at "+
  ...
  (string)params[1]->query_cap_name()+" is bombarded with "+
   "icicles.\n", params[1]);
 tell_object(params[1], (string)previous_object()->query_cap_name()+
  " points at you.\n"+"You are hit with a bombardment of icicles.\n");
 tell_object(previous_object(), "You point at "+
  (string)params[1]->query_cap_name()+".\n"+"You call upon the powers"+
  " of water to hex "+(string)params[1]->query_cap_name()+".\n"+
  (string)params[1]->query_cap_name()+" is hit with a bombardement of "+
  "icicles.\n");
params[1]->attack_by(previous_object());
/* exp... */
 hp = (int)params[1]->query_hp();
return (hp-(int)params[1]->adjust_hp(-40-params[0]));
