...
int hb_spell(object attacker, mixed *params)
{
 ...
 int i;
  ...
  damage = (int)this_player()->query_level();
 ...
 if (params[2] && (int)previous_object()->adjust_gp(-ADJ_COST)<0)
 {
  ...
  return 0;
   ...
   say(str+" chants, 'pyros burios'.\n", ({previous_object(),params[1]}));
  tell_object(previous_object(), "You chant, 'pyros burios'.\n");
  ...
  tell_object(previous_object(), "You envelope " + str + " in a Fire "+ 
	      "Burst!\n");
