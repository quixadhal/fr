/*** Major Acid Protection Spell ***/
/*** Created by Eerevann Tokani ***/


#define SP_NAME "Major Acid Resistance"
#define GP_COST 14
#define TYPE "acid"
...
int hb_spell(object attacker, mixed *params)
{
  ...
  say((string)previous_object()->query_cap_name()+
   " chants, 'marj corozev rissas'.\n", ({params[1],previous_object()}));
    
  if(params[1] == previous_object())
   { 
    tell_object(params[1], "You chant, 'marj corozev rissas'.\n");
  ...
  {
   tell_object(previous_object(), "You chant, 'marj corozev rissas'.\n");
   
   tell_object(params[1], (string)previous_object()->query_cap_name() +
    " chants, 'marj corozev rissas'.\n");
   
   tell_object(params[1], "You feel resistant to Acid attacks.\n");
