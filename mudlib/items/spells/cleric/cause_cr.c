...
#define SP_NAME "Cause Critical Wounds"
#define GP_COST 10
...
string help() {
       ...
       "Saving Throw: None\n"+
       "Description: \n"+
       "    This spell will do 3-24 + 2 points of damage to an enemy of your "+
...
thaco = (int)this_player()->query_thac0();
 roll = random(20)+1;
 ac_hit = thaco-roll;
...
/*** TELLS ROOM WHAT THE CASTER CHANTS, NOT HEARD BY CASTER OR TARGET ***/
  ...
  else
  {
   ...
   tell_object(previous_object(), "You chant, 'magius criusis harmei'.\n");
   
    /*** TELL TARGET WHAT CASTER CHANTS ***/
   tell_object(params[1], (string)previous_object()->query_cap_name() +
    " chants, 'magius criusis harmei'.\n");
   
   if((random((int)previous_object()->query_thac0()) + params[1]->query_total_ac()) > 0)
   {
    /*** TELL TARGET WHAT HAPPENS TO HIM AFTER SPELL GOES OFF ***/
   tell_object(params[1], (string)previous_object()->query_cap_name()+
      " reaches out and touches you as several lashes and bruises appear on you.\n");

    /*** TELL CASTER THE EFFECTS OF HIS SPELL AND WHO IT HIT ***/
   tell_object(previous_object(), "You cast "+SP_NAME+" and touch "+
    (string)params[1]->query_cap_name() + " causing several lashes "+
     "and bruises.\n");
