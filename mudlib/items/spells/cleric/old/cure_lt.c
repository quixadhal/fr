...
#define SP_NAME "Cure Light Wounds"
#define GP_COST 2
...
string help() {
       ...
       "Saving Throw: None\n"+
       "Description: \n"+
       "    This spell will cure 1-8 points of damage of any target.\n\n";
...
/*** TELLS ROOM WHAT THE CASTER CHANTS, NOT HEARD BY CASTER OR TARGET ***/
  ...
  if(params[1] == previous_object())
   { 
    ...
    tell_object(params[1], "You chant, 'magius elis curis'.\n");
   tell_object(params[1], "You cure some of your minor wounds.\n");
    say((string)previous_object()->query_cap_name()+
      " cures some of "+this_player()->query_possessive()+" minor "+
      "wounds.\n");
   }
  else
  {
    /*** TELL CASTER WHAT HE CHANTS ***/
   tell_object(previous_object(), "You chant, 'magius elis curis'.\n");
   ...
   say((string)previous_object()->query_cap_name()+
    " casts a spell and cures some of "+params[1]->query_cap_name()+
      "'s wounds.\n",
       ({params[1],previous_object()}) );
  }
