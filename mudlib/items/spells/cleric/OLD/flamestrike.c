...
int cast_spell(string str)
{
 ...
 skill = (int)this_player()->query_skill_bonus("faith.spells.offensive.target");
  ...
  write("You start casting Flame Strike.\n");
  return 1;
}
...
mixed spell(string str, int skill, int cast)
{
  ...
  if (sizeof(ob))
    ...
    ob = 0;
  this_player()->add_spell_effect(1, "attack", "Flame Strike", this_object(),
...
int hb_spell(object attacker, mixed *params)
{
 ...
 say((string)previous_object()->query_cap_name()+" casts Flame Strike at "+
  (string)params[1]->query_cap_name()+".\n", params[1]);
 tell_object(params[1], (string)previous_object()->query_cap_name()+
  " casts a Flame Strike at you.\n");
 tell_object(previous_object(), "You cast Flame Strike at "+
  (string)params[1]->query_cap_name()+".\n");
