...
int cast_spell(string str)
{
 ...
 if (skill < MIN_LVL)
 {
  notify_fail("You are not skilled enough to cast Creeping Doom.\n");
  return 0;
 }
...
mixed spell(string str, int skill, int cast)
{
 ...
 this_player()->add_spell_effect(1, "offensive", "Creeping Doom", this_object(),
...
int hb_spell(object caster, mixed *params)
{
 ...
 if(!params[1])
 {
  tell_object(previous_object(), "Your doom fails, not finding its target.\n");
  return 0;
 }
 ...
 {
  tell_object(previous_object(),"You feel a dreadful DOOM approaching.\n");
  say((string)previous_object()->query_cap_name() +
   " mumbles some strange words, and you feel its an evil spell.\nRUN!!!\n",
   params[1]);
 }

 else
 {
  tell_object(previous_object(),"You cast Creeping Doom on "+
   (string)params[1]->query_cap_name()+".\n");
  say((string)previous_object()->query_cap_name()+ " mumbles something and a "+
   "dark cloud descends on "+(string)params[1]->query_cap_name()+", spelling "+
   (string)params[1]->query_possessive()+" DOOM.\n",
    params[1]);
  tell_object(params[1], (string)previous_object()->query_cap_name()+
   " mumles something and you feel a dreadful DOOM descend on you.\n");
 }

 devil = clone_object("/std/spells/cleric/doom_obj");
 devil->move(environment(params[1]));
 tell_room(environment(devil), "A "+devil->query_short()+
  " quickly forms in the room.\n");
