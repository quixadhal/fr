...
void do_spell(object ob)
{
   ...
   if( find_living((string)ob->query_name()) )
   {
	  ...
	  if(random((int)ob->query_dex()) < 8 )
	  {
	     ...
	     {
            if(!interactive(ob))
		   ob->adjust_hp(-(BLADE_DAMAGE*4));
            else ob->adjust_hp(-BLADE_DAMAGE);
	     }
         else 
         {
           if(!interactive(ob)) ob->adjust_hp(-BLADE_DAMAGE);
            else if(ob->query_hp() > BLADE_DAMAGE) ob->adjust_hp(-BLADE_DAMAGE);
         }
	  }
