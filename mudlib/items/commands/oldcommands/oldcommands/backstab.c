...
int backstab(string str) {
  ...
  skill = (s1 = (int)this_player()->query_skill_bonus("dex.thief.stealth"))+
          (int)this_player()->query_skill_bonus("str.melee.sharp");
  if (s1 < MIN_LVL) {
  ...
  if((string)weap->query_weap_skill() != "str.melee.sharp") {
