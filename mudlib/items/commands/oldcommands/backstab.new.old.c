...
int backstab(string str) {
   ...
   if (!str)
      {
      notify_fail("Who do you want to backstab ?\n");
      return 0;
   }
   if(str == (string)this_player()->query_name()){
      notify_fail("You cannot backstab yourself!\n");
      return 0;
   }

   skill = (s1 =
(int)this_player()->query_dex())+
   (int)this_player()->query_level();
   weap = (object *)this_player()->query_weapons();
   if (sizeof(weap)<1)
      {
   ...
   {
      if((string)weap[i]->query_attack_type_name()
            == "slashing")
      {
            ret = do_backstab(str,skill,1,weap[i]);
            if (stringp(ret)) {
                notify_fail(ret);
               return 0;
           }
   ...
   notify_fail("You don't have a sharp weapon in your hand.\n");
   return 0;
}
...
/* used to add spells in the attack procedure. */
    return "You are already attacking.\n";
  ...
  this_player()->add_spell_effect(1, "attack", "Backstab", this_object(),
                         "heart_beat_backstab", ({ skill/2, ob, cast, wpn }));
  return 1;
}
...
int heart_beat_backstab(object attacker, mixed *params) {
  ...
  wpn = (object *)previous_object()->query_weapons();
  if ((wpn[0] != params[3]) && (wpn[1] != params[3]))
   {
  ...
  stabber_lvl = (int)this_player()->query_level();
  if(stabber_lvl)
    tmpdam /= (stabber_lvl /2);
  dam = tmpdam;
