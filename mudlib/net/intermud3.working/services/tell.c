...
void eventReceiveTell(mixed *packet) {
    ...
    if( !(ob = find_player(who)) || !(ob->query_creator()) ||
      (int)ob->query_invis() ) {
...
void eventSendTell(string who, string where, string msg) {
    ...
    mud = (string)INTERMUD_D->GetMudName(where);
    if(!mud)
      mud = (string)INTERMUD_D->GetMudName(implode(explode(where,"_")," "));
    INTERMUD_D->eventWrite(({ "tell", 5, mud_name(), pl, mud,
