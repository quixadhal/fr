/* communicate.c
 * this is for players communicatios, as say, ahout and the channels.
 * 
 * Baldrick.
 */
...
/* Wonder if I can put there in wiz_communicate?
 *
*/
...
int do_tell(string arg, object ob, int silent) 
  ...
  if (!ob) {
    ...
    if (!ob) {
/* Might be intermud... */
      if (this_object()->query_creator() && sscanf(str, "%s@%s", word, word) == 2) {
...
int do_emote(string arg) 
  ...
  this_player()->adjust_time_left(-5);
  /* Not needed in emote, looks good, but lacks reality.
   * Baldrick, nov '94
  if (this_object()->query_volume(D_ALCOHOL))
    arg = drunk_speech(arg);
   */
...
int do_shout(string str) 
  ...
  event(users(), "person_shout", this_object()->query_cap_name()+
			" "+s+": ", str, cur_lang, this_object()->query_int());
  if (s1 != "yell") {
...
int do_echo(string str) {
  ...
  if(adjust_social_points(-ECHO_COST) < 0) {
    notify_fail(NO_POWER);
    return 0;
  }
  if (this_object()->query_volume(D_ALCOHOL))
...
// Player, guild, domain, cre, lord and god chat
/* Baldrick here; cleaned up this a bit. (rather, starting to)
 * and removed all chat code.
 * the { } 'ing is a mess!
 */
