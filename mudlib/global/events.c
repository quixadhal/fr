...
/* reports idle time to the player -- FS */
...
/* tells player that you are kinda busy but also says that you are not
   ignoring them when you don't answer right away -- Firestorm */
...
void event_person_shout(object ob, string start, string mess, string lang, int level)
  {
  if (ob == this_object()) return ;
  ...
  if (member_array(lang, languages) == -1) {
    ...
    mess = "/std/language"->scramble_sentence(mess,level, this_object()->query_int());
    start = start[0..strlen(start)-3]+" in "+lang+": ";
  efun::tell_object(this_object(), new_line + fix_string(sprintf("%s%-=*s\n",
       start, cols-strlen(start), mess)));
} /* event_person_shout() */
