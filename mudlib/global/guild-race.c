...
void race_guild_commands() 
  ...
  /* wow, found it at last, this won't be used, yet
...
int do_command(string str) 
  {
  int i;
   if(this_player()->query_property("dead") || this_player()->query_property("noguild") || environment(this_player())->query_property("noguild") )
   {
...
void set_guild_ob(string str) 
  {
  /* this is *not* the way to test this...
  object boo;
  
  if (!boo = restore_object(str))
    {
    tell_object(this_object(), "BoooHiss.. this object is FoooBared..\n");
    return;
    }
  */
  guild_ob = str;
  this_object()->set_extreme_str(0);
  this_object()->set_thac0(22);
  guild_joined = time();
} /* set_guild_ob() */
