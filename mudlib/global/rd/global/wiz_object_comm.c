...
add_action("get_pathof","path*of");
  add_action("get_creator","crea*tor");
...
int teleport(string str) {
  ...
  if (dest) {
    ...
    if (!dest) {
      ...
      if (dest == environment()) {
        notify_fail("You look around and realize you are already there.\n");
        return 0;
      }
      this_player()->move_player("X", dest);
      return 1;
...
int get_pathof(string str) {
   ...
   for (i = 0; i < sizeof(ov); i++) {
      if(!ov[i] || ov[i]->query_hidden()) continue;
/*
      if (sizeof(ov) > 1) {
 */
...
check = find_player(str);
if(check){
  log_file("DEST", sprintf("Dested "+check->query_cap_name()+": %15-s %s\n",
...
/* It has already printed an error, so we return 1... */
  if (!junk) return 1;
  log_file("CALL",ctime(time())+" "+this_player()->query_name()+" issued the parse command :"+str+"\n");
...
log_file("CALL", ctime(time())+" "+this_player()->query_name()+" calls : "+ 
          str+"\n");
