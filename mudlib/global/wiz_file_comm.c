...
static void wiz_commands() {
  ...
  add_action("execute", "exe*c",100000);
...
/* Commented out and ghost's one put in its place. 
static int old_mv_file(string str) {
...
static int mv_file(string str) {
  ...
  string *filenames, dest, *fnames, source, trash1, trash2, trash3, trash4;
...
static int execute(string str) {
  ...
  if (file_size(wiz_dir)!=-2) {
   ...
   log_file("EXECS",(string)this_player()->query_cap_name()+" Exec'd |"+str+"|\n");
