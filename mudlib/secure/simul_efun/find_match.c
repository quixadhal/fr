/* modified December 1994, Taniwha, so that it will return a */
/* list of objects with NO duplicates in the list */
static mixed *rest;

/* If anyone can tell me what this does...
 * I would be most apprecative, Pinkfish... Yes yes ok i did write it.
 */
mixed old_find_match(string find,mixed in);
mixed find_match(string find,mixed in)
{
mixed *olist;
   int i,j;
   olist = old_find_match(find,in);
...
mixed old_find_match(string str, mixed ob) {
...
string nick, type, *bits;
  ...
  for (j=0;j<sizeof(bits);j++) {
    ...
    test = explode(str, " ");
    if(sizeof(test)) sscanf(test[sizeof(test)-1], "%d", bing);
    test = ({ });
    rest = ({ });
