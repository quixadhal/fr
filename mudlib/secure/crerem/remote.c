...
static private void eventProcess(int fd, string str) {
    ...
    if( !Connections[fd]["object"] ) {
        ...
        if( sscanf(arg, "%s %s", username, pass) != 2 ) {
        ...
        restore_object("/players/"+username[0..0]+"/"+username);
        ...
        case "edit":
          ...
          if (file == "" && (string)Connections[fd]["object"]->
              eventWriteFile(arg,"")[0..1] == "50") {
            eventWrite(fd, "50 " +cmd+ " " +arg+ ": Permission denied.\n");
            break;
          }
          if( file == "" ) file += "\n";
          if( file[<1] != '\n' ) file += "\n";
