...
void af_reset()
{
   ...
   i = sizeof(find_match("bastard swords", this_object()));
   for (;i<4;i++)
   {
      torch = clone_object("/baseobs/weapons/bastard_sword");
     torch->move(this_object());
   }
   if(!(find_match("flail",this_object())))
    {
     torch=clone_object("/baseobs/weapons/flail.c");
     torch->move(this_object());
    }
  if(!(find_match("platemail",this_object())))
   {
    torch=clone_object("/baseobs/armours/platemail");
    torch->move(this_object());
   }
  if(!(find_match("bracers",this_object())))
   {
    torch=clone_object("/baseobs/armours/bracers");
    torch->move(this_object());
   }
}
