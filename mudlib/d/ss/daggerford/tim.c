...
set_long("You are on Wall street. "+
	 "Lady Luck Tavern is to the north.  The southern gate is " +
         "to the south, as you'd probably expect. Wall street "+
         "follows the city walls to the "+
	 "east and west.\n");
...
void init()
{
  ...
  for (i=0;i<sizeof(MEMBERS);i++)
    if (this_player()->query_short()==MEMBERS[i])
   {
    tell_object(this_player(),"A voice thunders in your mind: TYPE DOWN TO ENTER THE HALL OF THE SILVER FIST!\n");
    add_action("go_hall","down");
   }
  ::init();
}
