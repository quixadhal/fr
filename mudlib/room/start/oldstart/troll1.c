inherit "/std/room";
object booklet,sword,chainmail;
...
int do_search(string str)
{
   if ((!sword) || (!chainmail))
   {
   sword=clone_object(DAGG +"bsword");
   sword->move(this_player());
  chainmail=clone_object(DAGG +"chainmail");
   chainmail->move(this_player());
   write("You find a Large Chainmail suit and large rusted sword, these "+
              "are well rusted and worn, but may be of some use.\n");
   tell_room(environment(this_player()), "searches arround and finds a "+
                  "rusted chainmail and battered sword.\n", (this_player()));
   return 1;
   }
